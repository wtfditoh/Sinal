...
// ⚠️ IMPORTANTE: essa página roda no GitHub Pages, mas a função de IA roda no
// Netlify (endereços diferentes). Troca a linha abaixo pelo endereço real do
// seu site no Netlify (aquele que aparece quando você publica o Studio lá).
const NETLIFY_FUNCTION_URL = "https://SEU-SITE-AQUI.netlify.app/.netlify/functions/ia";
...
const iaHistoricoContainer = document.querySelector("#iaHistorico");
const iaLimparHistoricoBtn = document.querySelector("#iaLimparHistoricoBtn");

const CHAVE_HISTORICO = "sinal_studio_ia_historico";
...
/* ---- Histórico: fica salvo no navegador, sobrevive a reload ---- */

function iaCarregarHistorico() {
...
function iaSalvarNoHistorico(item) {

  const historico = iaCarregarHistorico();
...
function iaFormatarDataHistorico(timestamp) {

  const agora = Date.now();
  const diffMin = Math.round((agora - timestamp) / 60000);
...
function iaRenderHistorico() {

  const historico = iaCarregarHistorico();

  if (historico.length === 0) {
    iaHistoricoContainer.innerHTML = '<p class="ia-historico-vazio">Nada gerado ainda. Suas legendas vão aparecer aqui.</p>';
    return;
  }

  iaHistoricoContainer.innerHTML = historico.map((item, i) => `
    <div class="ia-historico-item ${i === 0 ? "mais-recente" : ""}" style="animation-delay:${i * 0.03}s;">
      <div class="ia-historico-meta">
        <span class="ia-historico-tag">${escapeHtml(item.tag)}</span>
        <span>${iaFormatarDataHistorico(item.data)}</span>
      </div>
      <div class="ia-historico-texto">${escapeHtml(item.resposta)}</div>
      <div class="ia-historico-acoes">
        <button type="button" class="ia-historico-copiar" data-idx="${i}">📋 Copiar</button>
      </div>
    </div>
  `).join("");

  iaHistoricoContainer.querySelectorAll(".ia-historico-copiar").forEach((btn) => {

    btn.addEventListener("click", () => {

      const item = iaCarregarHistorico()[Number(btn.dataset.idx)];
...
iaLimparHistoricoBtn.onclick = () => {

  if (!confirm("Apagar todo o histórico de legendas geradas?")) return;
...
// Renderiza assim que a página carrega, pra recuperar o que já tinha
iaRenderHistorico();


// Faz a chamada de verdade pra Netlify Function - reaproveitada pelos dois modos
async function iaGerar(prompt, botao, span, textoOriginal, tag) {
  ...
  try {
    ...
    iaHistoricoContainer.scrollIntoView({ behavior: "smooth", block: "nearest" });
...
Escreva a legenda agora, seguindo o formato e o estilo acima.`;
  ...
  iaGerar(prompt, gerarLegendaCultoBtn, gerarLegendaCultoTexto, "✨ Gerar legenda", "🎤 Legenda de culto");
...
iaGerarBtn.onclick = () => {
  ...
  iaGerar(iaInput.value.trim(), iaGerarBtn, iaGerarTexto, "✨ Gerar", "💬 Livre");
