...
function carregarProximoCulto() {
  ...
  onSnapshot(q, (snapshot) => {
    ...
    if (contagem) {
      if (diffDias === 0) { contagem.textContent = "HOJE"; contagem.style.color = "var(--red)"; box.classList.add("hoje"); }
      else if (diffDias === 1) { contagem.textContent = "AMANHÃ"; contagem.style.color = "var(--amber)"; box.classList.remove("hoje"); }
      else { contagem.textContent = `EM ${diffDias} DIAS`; contagem.style.color = "var(--amber)"; box.classList.remove("hoje"); }
    }
    ...
    // Dia da semana + horário (se tiver)
  ...
  docs.forEach((docSnap, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="culto-corpo">
        ...
        <div class="checklist-dia" style="margin-top:8px;">
      ...
      <div class="culto-status-label ${isPostado ? "postado" : "pendente"}">${isPostado ? "postado" : "pendente"}</div>
    `;
